...
public  class RunExperiment_Bananagrams {
	...
	//hashtables to store results per strategy, the keys are a strategy together with an order
	//The values are the corresponding statistic
	...
	private static Hashtable<String,Long> maxTimeElapsedLocal= new Hashtable<String,Long>();
	...
	public static void runExperiment(int order){
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				System.out.println("Working on Problem number"+counter);
	...
	public static void runSearches(Problem thisProblem, double counter, int order){
		...
		for (SearchEngine engine : engines){
			...
			System.out.println(engine.getStrategy());
	...
	public static void updateLocalStats(String key, SearchEngine engine,double n ){
		...
		updateAverage(avgdepthOfSolutionLocal,engine.getDepthOfSolution(),n,key);
		...
		if ( !maxTimeElapsedLocal.containsKey(key)){
			maxTimeElapsedLocal.put(key, engine.getTimeElapsed());
